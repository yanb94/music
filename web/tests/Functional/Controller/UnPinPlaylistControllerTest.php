...
class UnPinPlaylistControllerTest extends ApiTestCase
{
    ...
    private function getRequestUnPinPlaylist(Client $client, string $slug, string $token): ResponseInterface
    {
        return $client->request(
            'GET',
            "/api/playlists/" . $slug . "/unpin",
            [
                "headers" => [
                    "Accept" => "application/ld+json",
                    'Content-Type' => 'application/json'
    ...
    public function testPlaylistIsPinned(): void
    {
        ...
        $this->getRequestUnPinPlaylist($client, 'playlist-1', $token);
        $this->assertResponseStatusCodeSame(200);
    ...
    public function testWhenNoAuth(): void
    {
        ...
        $client = $this->testClient;
        $this->getRequestUnPinPlaylist($client, 'playlist-1', '');
