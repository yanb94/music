...
async function renderMainElement(moreAction = () => {}, isMore = false, className = 'test')
{
    ...
    await act(async () => {
        renderElements = render(<ListPlaylists className={className} isMore={isMore} moreAction={moreAction}/>,{wrapper: BrowserRouter})
    })

    return renderElements
}

function mockGetPlaylists(result)
{
    getPlaylists.mockImplementation(() => Promise.resolve(result))
}
...
it("lists are rendered when request success", async() => {
    ...
    mockGetPlaylists(fakeDataPlaylist(8))
